...
def main():
    ...
    results_folder = create_results_folder(data_folder)
    ...
    if data_files:
        ...
        process_sample_visualization(sample_data, sample_file, results_folder)
    
    # Step 5: Analyze all files
    print("\nAnalyzing all files...")
    results_df = analyze_all_files(data, data_files, results_folder)
    ...
    summary_plot_path = os.path.join(results_folder, "summary_dashboard.png")
    plt.savefig(summary_plot_path, dpi=300, bbox_inches='tight')
    plt.close()  # Close the figure to free memory
    print(f"Summary dashboard saved to: {summary_plot_path}")
    
    print(f"\nAnalysis complete! All results saved to: {results_folder}")
